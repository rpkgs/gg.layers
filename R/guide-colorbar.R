...
#' @importFrom ggplot2 guide_train guide_merge guide_gengrob
...
width_cm               = ggplot2:::width_cm
height_cm              = ggplot2:::width_cm
ggname                 = ggplot2:::ggname
justify_grobs          = ggplot2:::justify_grobs
matched_aes            = ggplot2:::matched_aes
include_layer_in_guide = ggplot2:::include_layer_in_guide
compact                = ggplot2:::compact

#' @export
guide_train.colorbar2 <- function(guide, scale, aesthetic = NULL) {
...
guide_gengrob.colorbar2 <- function(guide, theme) {
  ...
  if (guide$raster) {
    ...
    if (guide$direction == "horizontal") {
      ...
      key.layout = grid.layout(
        ...
        heights = c(bh, (n - 2) * bh, bh), widths = barwidth, just = c(0, 0)
      )
      frame <- frameGrob(layout = key.layout)

      g_bar = rectGrob(
        x = 0, y = by[1:(n-2)], vjust = 0, hjust = 0,
        width = barwidth, height = bh, default.units = "cm",
        gp = gpar(col = NA, fill = guide$bar$colour[2:(n-1)])
      )
      # Ipaper::write_fig(g_bar)
      l = make_triangle(cols = guide$bar$colour)

      frame %<>% placeGrob(g_bar, row = 2, col = 1)
      frame %<>% placeGrob(l$lower, row = 3, col = 1)
      frame %<>% placeGrob(l$upper, row = 1, col = 1)
      # vp = viewport(xscale = , yscale = )
      # 这里添加triangle
      grob.bar <- frame
    }
  }
  ...
  tick_pos <- scales::rescale(guide$key$.value, c(0.5, guide$nbin - 0.5), guide$bar$value[c(1, nrow(guide$bar))]) * barlength / guide$nbin
  ...
  just_defaults <- label_just_defaults.colorbar2(guide$direction, label.position)
...
make_triangle <- function(space = "right", cols) {
  ...
  gp_upper <- gpar(fill = cols[n], col = col, alpha = alpha, lwd = lwd)
